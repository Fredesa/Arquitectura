      ...
      <div class="card text-center" style="width: 20rem; border-radius: 20px">
        <div class="card-body">
          ...
          <form @submit.prevent="Login">
            ...
            <router-link
              to="/reset-password"
              style="text-decoration: none; font-size: medium; color: grey"
            >
              Recuperarla</router-link
            >
          </form>
        </div>
      </div>
  ...
  setup() {
    ...
    const Login = () => {
      firebase
        ...
        .then((data) => {
          if (data.user.uid == "W8930eacw6dYAlt0FlCATBwZdxx1") {
            router.replace("/admin");
          } else {
            console.log(data), router.replace("/medico");
          }
        })
